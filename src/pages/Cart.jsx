...
export default function Cart() {
  ...
  useEffect(() => {
    // за бажанням: автопідстановка з профілю
  }, [])
  ...
  async function checkout(e) {
    ...
    setLoading(false)
    if (error) return alert('Помилка створення замовлення: ' + error.message)
      ...
      {items.length > 0 && (
        <>
          <div style={{ display: 'grid', gap: 8, marginBottom: 16 }}>
            {items.map(i => (
              <div
                ...
                style={{
                  display: 'grid',
                  gridTemplateColumns: '80px 1fr 140px 150px 40px',
                  gap: 8,
                  alignItems: 'center',
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#777' }}>
                    Дроп-ціна: {Number(i.price_dropship).toFixed(2)} ₴
                  </div>
                </div>
          ...
          <form onSubmit={checkout} style={{ display: 'grid', gap: 12, maxWidth: 520 }}>
            ...
            />
            <input
              placeholder="Телефон отримувача (+380...)"
            ...
            <button type="submit" disabled={loading}>
              {loading ? 'Створюємо...' : 'Підтвердити замовлення'}
            </button>
          </form>
        </>
      )}
...
/** Акуратний хрестик з підсвіткою на hover */
function DeleteButton({ onClick }) {
  ...
  return (
    <button
      ...
      style={{
        ...base,
        color: hover ? '#c1121f' : '#333',
        borderColor: hover ? '#c1121f' : '#ddd',
        background: hover ? '#fff5f5' : '#fff',
      }}
      aria-label="Видалити"
    >
      ×
