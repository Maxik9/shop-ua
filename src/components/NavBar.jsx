  ...
  return (
    <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
      <div className="max-w-6xl mx-auto px-3 h-14 flex items-center gap-3">
        {/* Бургер */}
        <button
          className="order-1 md:hidden w-10 h-10 rounded-lg border border-slate-300 flex items-center justify-center"
          onClick={() => setOpen(v => !v)}
          aria-label="Меню"
        >
          {!open ? (
        ...
        {/* Пошук (десктоп) */}
        <form onSubmit={submitSearch} className="order-4 hidden md:block md:ml-auto">
        ...
        <div className="order-5 ml-auto flex items-center gap-2">
          {/* кнопка пошуку на мобільному */}
          <Link to="/search" className="order-1 md:hidden w-10 h-10 rounded-lg border border-slate-300 flex items-center justify-center" title="Пошук">
